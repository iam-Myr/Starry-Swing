...
@onready var camera = get_tree().current_scene.get_node("Player/PlayerCamera")
@export var sprite_scale = 4
@export var y_mirroring = false
...
func _process(_delta):
	...
	if screen_size != last_screen_size:
		...
		var rect = Rect2(0, 0, sprite_size.x * ceil(screen_size.x / sprite_size.x/sprite_scale),sprite_size.y)
